---
title: Bug Sprite Sheet
comments: true
layout: base
description: Use JavaScript to display bug animations from a sprite sheet.
permalink: /frontend/bugs
...
{% assign hash = site.data.bugs %}  <!--- Liquid list variable created from file containing mario metatdata for sprite--->
{% assign pixels = 250 %} <!--- Liquid integer assignment --->
...
<style>
  ...
  */
  .sprite {
    ...
    height: {{pixels}}px;
    width: {{pixels}}px;
    transform: scale(1);  /* scales the display size of sprite frame in HTML */
    font-size: 2em;
    text-align: center;
  }
